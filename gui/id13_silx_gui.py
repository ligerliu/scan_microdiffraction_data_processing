...
from Iq_analysis_UI import Iq_analysis
from XRF_load_UI import XRF_UI
from colormap_setup import setup_colormap
from total_sum_UI import total_sum                
...
class input_window(QMainWindow):
    ...
    def input_UI(self):
        ...
        self.path_input = QLineEdit('/gpfs/jazzy/data/id13/inhouse3/DATAPOLICY_I3_1/eh3/inhouse/ihsc1698/id13',path)
        ...
        # here are functions for process and analysis
        ...
        Iq_ana_proc = QPushButton("Iq ana",proc_widget)
        Iq_ana_proc.setGeometry(80,70,60,20)
        Iq_ana_proc.clicked.connect(self.Iq_ana_proc)
        
        pyfai_run = QPushButton("run pyFAI",proc_widget)
        pyfai_run.setGeometry(10,100,60,20)
        pyfai_run.clicked.connect(self.run_pyFAI)
        
        single_pttn_ana = QPushButton('pttn ana',proc_widget)
        single_pttn_ana.setGeometry(80,100,60,20)
        single_pttn_ana.clicked.connect(self.run_single_pttn_ana)
        
        XRF_load = QPushButton("load XRF",proc_widget)
        XRF_load.setGeometry(10,130,60,20)
    ...
    def add_file(self):
        try:
            ...
            with h5py.File(path,'r') as f:
                ...
                check_list = self.tree.findItems(self.filename,Qt.MatchExactly)
                if len(check_list) == 0:
                    self.tree_root = QTreeWidgetItem(self.tree,
                                    [self.filename])
                # the update problem have been partially solved
                # need to test whether it will update the h5py info 
    ...
    def add_branch(self,tree_root,h5file):
        children = []
        for child_idx in range(tree_root.childCount()):
            children.append(tree_root.child(child_idx))
        for child in children:
            tree_root.removeChild(child)
        for _ in h5file:
            try:
    ...
    def onItemClicked(self,item):
        ...
        #      #self.tree_root.currentItem().text(0),
        #     '\n############')
        try:
    ...
    def total_sum_proc(self):
        try:
            self.total_sum_win = total_sum(self.scan_obj)
                        #self.h5_list,
                        #self.path_idx,self.pttn_idx,
                        #self.scan_shape)
            self.total_sum_win.show()
        except Exception as e:
            self.proc_msg.setText(str(e))
            pass
    ...
    def Iq_ana_proc(self):
        try:
            self.Iq_ana_win = Iq_analysis()
            self.Iq_ana_win.show()
        except Exception as e:
            self.proc_msg.setText(str(e))
            pass
    def run_pyFAI(self):
    ...
    def run_single_pttn_ana(self):
        try:
            os.system('python single_pattern_process_Iq.py')
        except Exception as e:
            self.proc_msg.setText(str(e))
            pass 
