...
def save_qphi_as_h5(obj,save_path,name,
    ...
    with h5py.File(h5_name,'a') as f:
        ...
        idx_list = chunk_idx(int(patch_pttn_idx[1]-patch_pttn_idx[0]),
                            single_h5_pttn_num)
        proc_h5_name_list = []
        if patch_idx[0] == 0:
            pass
        else:
            for ll in fc.attrs["proc_h5_list"]:
                proc_h5_name_list.append(ll)
        ...
        for _ in range(patch_idx[0],patch_idx[1]):
            ...
            with h5py.File(proc_h5_name,'a') as k:
                ...
                Iq_data = []
                for __ in range(idx_list[int(_-patch_idx[0])][0],idx_list[int(_-patch_idx[0])][1]):
    ...
    print("file {:05d} to {:05d} save finished".format(patch_idx[0],patch_idx[1]))
...
def auto_proc_Iq(obj,
              ...
              radial_range = None,
              patch_size = 400,#too much hdf files to process, try to process every 400 files to save memory
              **kwargs
              ):
    failed_pattern = []
    for ii in samples.keys():
        #try:
            ...
            if not isinstance(obj.ct34, type(None)):
                ...
                ct34 = np.ones((len(pttn_idx.flatten()),))
            patch_list = chunk_idx(len(h5_list),patch_size)
            patch_pttn_list = chunk_idx(total_pttns,patch_size*obj.single_h5_shape[0])
            for i in range(len(patch_list)):
                patch_idx = patch_list[i]
                patch_pttn_idx = patch_pttn_list[i]
                res = parallel_func(scan_calculate_Iq,
                           num_core,
                           np.arange(patch_pttn_idx[0],patch_pttn_idx[1]),
                           h5_list   = h5_list,
